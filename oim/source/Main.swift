...
enum OimApplicationDirectory {

    /// MARK: - Constant
    private static let RIME: String = "rime"
    private static let LIBRARY: String = "Library"

    /// MARK: - User 目录 ~/Library/rime
    ...
    /// MARK: - App 目录 /Library/InputLibrary/Oim.app
    static let appDirectory: URL = "/Library/InputLibrary/Oim.app".withCString { dir in
        URL(
            fileURLWithFileSystemRepresentation: dir,
            isDirectory: false,
            relativeTo: nil
        )
    }

    /// MARK: - Log 目录 /tmp/rime.oim
    static let logDirectory: URL = FileManager.default.temporaryDirectory.appending(
        component: "rime.oim",
...
struct OimApplication {
    ...
    static func main() {
        ...
        let handled = autoreleasepool {
            ...
            if args.count > 1 {
                ...
                case "--enable-input-source":
                    if args.count > 2 {
                        let modes = args[2...].map { SquirrelInstaller.InputMode(rawValue: $0) }
                            .compactMap {
                                $0
                            }
                        if !modes.isEmpty {
        ...
        autoreleasepool {
            ...
            let connectionName =
                main.object(forInfoDictionaryKey: "InputMethodConnectionName") as! String
            ...
            if NSApp.squirrelAppDelegate.problematicLaunchDetected() {
                ...
                task.executableURL = "/usr/bin/say".withCString { dir in
                    URL(
                        fileURLWithFileSystemRepresentation: dir, isDirectory: false,
                        relativeTo: nil)
                }
                task.arguments = args
                try? task.run()
